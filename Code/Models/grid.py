...
class Grid(nn.Module):
    def __init__(self, opt):
        ...
        g = torch.rand(grid_shape, 
            dtype=torch.float32, 
            device=opt['device']) * 0.1
    ...
    def forward_grad(self, coords):
        grad_grid = jacobian(self.grid, normalize=False)[:,0]
        output = grid_sample_3d(grad_grid, coords.unsqueeze(1).unsqueeze(1).unsqueeze(0)).squeeze().permute(1,0)
        return output
